...
class PHPWord_Template {
	...
	*/
	...
	public function setValue($search, $replace, $limit=-1) {
		...
		preg_match_all('/\{[^}]+\}/', $this->_documentXML . $this->_footer1XML, $matches);
		foreach ($matches[0] as $k => $match) {
			$no_tag = strip_tags($match);
			if ($no_tag == $search) {
				$match = '{'.$match.'}';
				$this->_documentXML = preg_replace($match, $replace, $this->_documentXML, $limit);
                $this->_footer1XML = preg_replace($match, $replace, $this->_footer1XML, $limit);
				if ($limit == 1) {
					break;
				}			
			}
	...
	*/
     ...
     * @param integer $limit
     */
    public function removeRow($search, $limit=-1) {
        // read document as XML
        $xml = new DOMDocument;
        $xml->loadXML($this->_documentXML, LIBXML_NOENT | LIBXML_XINCLUDE | LIBXML_NOERROR | LIBXML_NOWARNING);
        ...
        foreach ($tables as $table) {
            ...
            // search for pattern. Like {TBL1.
            if (mb_strpos($text, '{'.$search.'.') !== false) {
                // search row for remove
                $rows = $table->getElementsByTagName('tr');
                foreach ($rows as $row) {
                    $text = $row->textContent;
                    if (
                        mb_strpos($text, '{'.$search.'.') !== false // Pattern found in this row
                        AND ($countRemoved < $limit OR $limit < 0)
                    ) {
