<div class="popup-register-cont">
	<div class="popup-register-block auth">
		...
		<div class="popup-register-header"><?= Yii::t('Front', 'SMS registration') ?></div>
			<?php $form=$this->beginWidget('CActiveForm', array(
						...
						'afterValidateAttribute' => 'js:function(form, attribute, data, hasError) {
						   if(hasError) {$("#"+attribute.id).addClass("input-error");$("#"+attribute.id).next(".validation-icon").show();}
                            ...
                            <div class="popup-register-form sms-form" id="">
                                <div class="form-line">
                                    <div class="form-block">
                                        ...
                                        <div class="sms-hint"><?= Yii::t('Front', 'We have send an SMS with the verification code on the phone number +x xxx xxx :num.', array(':num' => substr($user->phone, -3))); ?></div>
                                ...
                                <div class="register-forgot-row">
                                    <a onclick="resendLoginEmail('<?= Yii::t('Front', 'SMS was sent') ?>', '<?= Yii::app()->createUrl('/site/ResendVerifySMS') ?>')" href="javaScript:void(0)"><?= Yii::t('Front', 'I haven\'t received SMS with the code. Send it again.') ?></a>

                                    <div class="change-phone-cont">
                                        <?php if(!$user->phone_confirm): ?>
                                            <a href="<?= Yii::app()->createUrl('/site/SMSVerifyPhoneChange') ?>"><?= Yii::t('Front', 'The mobile phone number is incorrect. Change mobile phone number.') ?></a>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </div>
