 ...
 * @since 1.0
 */
class QFileLogRoute extends CLogRoute
{
	/**
	...
	protected function processLogs($logs)
	{
		...
		foreach($logs as $log){
			$logFileName = $log[2];
			if(!isset($files[$logFileName])){
				$logFile=$this->getLogPath().DIRECTORY_SEPARATOR . $logFileName . '.log';
				if(@filesize($logFile)>$this->getMaxFileSize()*1024)
					$this->rotateFiles();
				$files[$logFileName]=@fopen($logFile,'a');
				@flock($files[$logFileName],LOCK_EX);
			}
				@fwrite($files[$logFileName],$this->formatLogMessage($log[0],$log[1],$log[2],$log[3]));
