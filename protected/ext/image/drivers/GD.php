...
class Image_GD_Driver extends Image_Driver {
	...
	public function addWatermark($text)
	{
		...
		$font_size = 10;
		$withtext = strlen($text) * 6.5;
		$background = imagecreatetruecolor($withtext, 20);
		$black = imagecolorallocate($background, 0, 0, 0);
		imagecopy($image_p, $background, $width - $withtext - 10, $height - 25, 0, 0, imagesx($background), imagesy($background));
		imagettftext($image_p, $font_size, 0, $width - $withtext, $height - 10, $white, $font, $text);
		
		imagedestroy($this->tmp_image);
		$this->tmp_image = $image_p;
		return true;
	}
