...
class CustomLinkedinOAuthService extends LinkedinOAuthService {
	...
	protected function fetchAttributes() {
		$info = $this->makeSignedRequest('http://api.linkedin.com/v1/people/~:(id,first-name,last-name,public-profile-url,picture-url)', array(), false); // json format not working :(
		$info = $this->parseInfo($info);

		$this->attributes['soc_id'] = $info['id'];
		$this->attributes['full_name'] = $info['first-name'].' '.$info['last-name'];
		$this->attributes['url'] = substr($info['public-profile-url'], 11);
        $this->attributes['avatar'] = '';
		$this->attributes['login'] = $this->attributes['full_name'];
        if(isset($info['picture-url'])){
            $this->attributes['avatar'] = $info['picture-url'];
        }
	}
